# app.py — Streamlit PdM Dashboard (Mock-up) WITHOUT scikit-learn
import os
import numpy as np
import pandas as pd
...
def safe_read_csv(f, nrows=None):
    try:
        ...
        return pd.read_csv(f, encoding="latin-1", nrows=nrows)
...
def load_demo():
    ...
    vib[300:] += np.linspace(0.0, 0.6, n-300)
...
def compute_confusion_matrix(y_true, y_pred, labels=(1,0)):
    """Simple confusion matrix without scikit-learn."""
    # rows = actual labels order; cols = predicted labels order
    cm = np.zeros((len(labels), len(labels)), dtype=int)
    for i, a in enumerate(labels):
        for j, p in enumerate(labels):
            cm[i, j] = np.sum((y_true == a) & (y_pred == p))
...
if uploaded is not None:
    df = safe_read_csv(uploaded, nrows=50000)
elif repo_path is not None:
    st.info(f"Attempting to load: **{repo_path}**")
    df = safe_read_csv(repo_path, nrows=50000)

if df is None:
    st.warning("Using built-in demo data (add or upload 'unified_sensor_Dataset 2.csv' to replace this).")
    df = load_demo()
...
default_vib   = guess(["vibration rms - motor 1v", "vib rms", "vib acc rms", "vibration"])
default_temp  = guess(["surface temperature", "temp"])
default_rpm   = guess(["motor speed", "rpm", "hz", "frequency"])
default_label = None  # you likely don't have it yet
...
with c3:
    label_col = st.selectbox("Label column (0=normal,1=failure, optional)", options=["(none)"] + cols, index=(cols.index(default_label)+1 if default_label in cols else 0) if default_label in cols else 0)
